...
function GetElasticHelmholtzOperator(p::ElasticHelmholtzParam;spread::Bool = false,beta = [1.0;1.0])
...
function GetElasticHelmholtzOpReformulatedSpread(Mesh::RegularMesh,mu::Array,lambda::Array,rho::Array, omega::Float64, gamma::Array, beta)
	factor = 1.0/(sum(Mesh.h)/length(Mesh.h));
	factor = 1.0;
